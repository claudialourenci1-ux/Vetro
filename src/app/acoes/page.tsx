...
export default async function ActionsPage() {
  ...
  const { data, error } = await rpc.rpc('get_commercial_action_center', { target_company_id: workspace.company.id })
  ...
  return <AppShell companyName={workspace.company.name} role={workspace.membership.role} permissions={workspace.permissions}>
    <PageHeading eyebrow="Execução comercial" title="Plano de ação">
      {canManage ? <form action={refreshSignalsAction}><input name="from" type="hidden" value={iso(from)} /><input name="to" type="hidden" value={iso(today)} /><button className={styles.secondaryButton} type="submit"><RefreshCw size={15}/>Atualizar diagnóstico</button></form> : null}
    </PageHeading>
    ...
    {error ? <section className="workspace-error"><strong>Não foi possível carregar o plano de ação.</strong><span>{error.message}</span></section> : <>
      <section className={styles.summaryGrid}>
        <article><span>Ações abertas</span><strong>{open.length}</strong><ListChecks size={17}/></article>
        <article><span>Em andamento</span><strong>{doing.length}</strong><Clock3 size={17}/></article>
        <article><span>Concluídas</span><strong>{done.length}</strong><CheckCircle2 size={17}/></article>
        <article><span>Sinais ativos</span><strong>{signals.length}</strong><CircleAlert size={17}/></article>
      </section>

      <section className={styles.topGrid}>
        <article className={styles.panel}>
          <div className={styles.panelHeading}><div><span>VETRO INTELLIGENCE</span><h2>Pontos que pedem ação</h2></div><CircleAlert size={18}/></div>
          <div className={styles.signalList}>{signals.length ? signals.slice(0, 6).map((signal) => <div className={styles.signal} key={signal.id}><div><b className={`${styles.badge} ${styles[signal.severity ?? 'info']}`}>{signal.severity === 'critical' ? 'Crítico' : signal.severity === 'warning' ? 'Atenção' : signal.severity === 'opportunity' ? 'Oportunidade' : 'Informação'}</b><span>{signal.category}</span></div><strong>{signal.title}</strong><p>{signal.message}</p>{signal.recommended_action ? <small><b>Próximo passo:</b> {signal.recommended_action}</small> : null}</div>) : <p className={styles.empty}>Atualize o diagnóstico para a VETRO calcular sinais a partir dos dados reais da operação.</p>}</div>
        </article>

        <article className={`${styles.panel} ${styles.aiPanel}`}>
          <div className={styles.panelHeading}><div><span>VETRO AI</span><h2>Leitura executiva mais recente</h2></div><Sparkles size={18}/></div>
          {brief ? <><p className={styles.brief}>{brief.executive_summary || 'Briefing gerado sem resumo textual.'}</p><div className={styles.briefMeta}><span>{brief.model || 'OpenAI'}</span><span>{brief.generated_at ? dateTime.format(new Date(brief.generated_at)) : 'gerado recentemente'}</span></div>{canManage ? <form action={adoptBriefActionsAction}><input name="brief_id" type="hidden" value={brief.id}/><button className={styles.primaryButton} type="submit"><ListChecks size={15}/>Transformar recomendações em plano de ação</button></form> : null}</> : <p className={styles.empty}>Gere uma leitura no Overview. A IA interpreta o cockpit calculado pela VETRO, sem recalcular ou inventar KPIs.</p>}
        </article>
      </section>

      {canManage ? <section className={styles.panel}>
        <div className={styles.panelHeading}><div><span>NOVA AÇÃO</span><h2>Registrar iniciativa comercial</h2></div><UserRoundCheck size={18}/></div>
        <form action={createManualActionAction} className={styles.createForm}>
          <label className={styles.wide}><span>Título</span><input maxLength={160} name="title" placeholder="Ex.: Revisar propostas paradas há mais de 7 dias" required /></label>
          <label className={styles.wide}><span>Descrição</span><input maxLength={500} name="description" placeholder="Contexto e resultado esperado" /></label>
          <label><span>Prioridade</span><select defaultValue="medium" name="priority"><option value="low">Baixa</option><option value="medium">Média</option><option value="high">Alta</option><option value="critical">Crítica</option></select></label>
          <label><span>Responsável</span><select defaultValue="" name="assigned_to"><option value="">Não atribuído</option>{members.map((member) => <option key={member.user_id} value={member.user_id}>{member.name || 'Usuário'} · {member.role}</option>)}</select></label>
          <label><span>Prazo</span><input name="due_at" type="datetime-local" /></label>
          <button className={styles.primaryButton} type="submit">Criar ação</button>
        </form>
      </section> : null}

      <section className={styles.panel}>
        <div className={styles.panelHeading}><div><span>EXECUÇÃO</span><h2>Ações comerciais</h2></div><span className={styles.counter}>{actions.length}</span></div>
        <div className={styles.actionList}>{actions.length ? actions.map((action) => <article className={styles.actionCard} key={action.id}>
          ...
          <form action={updateCommercialActionAction} className={styles.actionForm}><input name="action_id" type="hidden" value={action.id}/>
        ...
        </article>) : <p className={styles.empty}>Nenhuma ação criada ainda. Sinais e recomendações podem ser transformados em execução sem sair da VETRO.</p>}</div>
      </section>
    </>}
  </AppShell>
}
